        ...
        if $?.success?
          ...
          instrument "ruby.bundle_clean" do
            ...
            if load_default_cache?
              run "env BUNDLE_CONFIG=/dev/null #{bundle_bin} clean > /dev/null"
              #run("#{bundle_bin} clean > /dev/null", user_env: true)
            else
              pipe("env BUNDLE_CONFIG=/dev/null #{bundle_bin} clean --no-dry-run", out: "2> /dev/null")
...
<% end %>
  ...
  def load_bundler_cache
    instrument "ruby.load_bundler_cache" do
      ...
      # mjt - 2013-09-06 - oopsie, Heroku devs used 'ruby_version_cache' below
      old_rubygems_version = @metadata.read(rubygems_version_cache).chomp if @metadata.exists?(rubygems_version_cache)
